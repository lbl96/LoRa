...
{ 
	setNSS(0);                      //拉低NSS
	sendSPIxByte(SPI2, addr|0x80);//最高位为1时表示写，再写一次时写寄存器
...
unsigned char readSX1278(unsigned char addr)
{
	...
	setNSS(0);                      //拉低NSS
	sendSPIxByte(SPI2, addr&0x7f);//最高位为0时表示读，之后用读函数读出值
	/* 最高位：
	   读指令（与）：0x80 - 1000 0000
	   写指令（或）：0x7f  - 0111 1111
	*/
...
void setSX1278OpMode( RFMode_SET opMode )
{
	...
	opModePrev &=0xf8;                        //清零低三位//与1时其他位不变，1还是1,0还是0，与0时等于是清空全为0
	opModePrev |= (unsigned char)opMode;      //或上要写的值
...
{	
	...
	/*地址字节之后跟有多个数据字节。该地址在各数据字节间自动内部递增。这
...
一直保持低位，仅在最后一个字节传输完后才拉高。*/
	...
	setNSS(1);                                       //拉高NSS
	writeSX1278(REG_LR_DIOMAPPING1,0x40);            //设置0x40寄存器为0100 0000b，即设置发射完成指示映射到DIO0引脚,RegDioMapping1 (0x40) 
	writeSX1278(REG_LR_DIOMAPPING2,0x00);
	setSX1278OpMode(Transmitter_mode); 

    while((readSX1278(REG_LR_IRQFLAGS)&0x08) != 0x08)//等待发送完成,第3位，不是第7位
		//FIFO负载发送完成中断：一次写操作清除IRQ
    {
        Delay_ms(20);
    }
